 ...
 * sdcard.c
 *
 * SD Card Raw Data read and write for AVR (ATtiny817)
 *
 * Created: 24-Jun-16 1:33:22 PM
...
/* CHECK THESE ARE RIGHT FOR YOUR DEVICE!!
 * Originally for ATtiny817 alternate SPI location
 */
...
#define SELECTING !(SPIPORT.OUT & SPI_CS)
...
static void init_spi(void)
{
	...
	/* Master Mode, 100-400kHz clk for initialization sequence */
	...
	/* Slave select line controlled in software */
...
DSTATUS disk_initialize(void)
{
	...
	if (send_cmd(GO_IDLE, 0) == 1) {
		...
		if (send_cmd(CHECK_V, 0x1AA) == 1) {
			...
			for (timer = 10000; timer && send_cmd(APP_INIT, (1UL << 30)); timer--) {
				_delay_us(100);
			}
			/* Check CCS bit */
	...
	if (type == 0) {
		...
		/* Change SPI clock to be faster (20MHz + DIV4 + CLKX2 = 10MHz) */
